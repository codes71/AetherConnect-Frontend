...
interface AuthApi {
  login: (data: Record<string, unknown>) => Promise<AxiosResponse<{ success: boolean; user: User }>>;
  register: (data: Record<string, unknown>) => Promise<AxiosResponse<{ success: boolean; user: User }>>;
  ...
  private setupInterceptors() {
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (!originalRequest) {
          return Promise.reject(this.normalizeError(error, 'request-config-missing'));
        }
